...
OBJS := \
	$(obj).target/$(TARGET)/sample/hello/hello.o
...
$(obj).target/sample/hello/libhello.so: GYP_LDFLAGS := $(LDFLAGS_$(BUILDTYPE))
$(obj).target/sample/hello/libhello.so: LIBS := $(LIBS)
$(obj).target/sample/hello/libhello.so: LD_INPUTS := $(OBJS)
$(obj).target/sample/hello/libhello.so: TOOLSET := $(TOOLSET)
$(obj).target/sample/hello/libhello.so: $(OBJS) FORCE_DO_CMD
	$(call do_cmd,solink)

all_deps += $(obj).target/sample/hello/libhello.so
# Add target alias
.PHONY: hello
hello: $(builddir)/lib.target/libhello.so

# Copy this to the shared library output path.
$(builddir)/lib.target/libhello.so: TOOLSET := $(TOOLSET)
$(builddir)/lib.target/libhello.so: $(obj).target/sample/hello/libhello.so FORCE_DO_CMD
	$(call do_cmd,copy)

all_deps += $(builddir)/lib.target/libhello.so
# Short alias for building this shared library.
.PHONY: libhello.so
libhello.so: $(obj).target/sample/hello/libhello.so $(builddir)/lib.target/libhello.so

# Add shared library to "all" target.
.PHONY: all
all: $(builddir)/lib.target/libhello.so
